...
AT_KRL_Parser.prototype.parseStFzFact = function (line) {
    ...
    if (line.indexOf('=') == -1 && line.indexOf('<') == -1 && line.indexOf('>') == -1) {
        throw new SyntaxError('Невозможно получить факт из объявления:\n' + line + '\nВыражения для символьных и нечетких значений должны сравниваться знаками "=", "<" и ">"')
    }
    var sings = ['=', '<', '>', '>=', '<='];
    var eq = -1;
    var sing = '=';
    for (var i = 0; i < sings.length; i++) {
        var tmp = line.indexOf(sings[i]);
        if (tmp != -1) {
            if ((tmp <= eq || eq == -1) && (sings[i].length == 1 || (sings[i].length == 2) && (tmp + 1 == line.indexOf('=')))) {
                eq = tmp;
                sing = sings[i];
            }
        }
    ...
    var r = line.substring(eq + sing.length, line.lastIndexOf(' УВЕРЕННОСТЬ ['));
    ...
    var f = new AT_KRL_Fact(left, right, sing, sure, exact);
    return f;
}
...
AT_KRL_Parser.prototype.getConditionFromTree = function (root) {
    ...
    if (!root.right) {
        return new AT_KRL_SimpleCondition(root)
    } else {
...
AT_KRL_Parser.prototype.parseRuleConditions = function (line) {
    ...
    var res = this.getConditionFromTree(tree);
	return res;
}
...
AT_KRL_Parser.prototype.parseRuleConditionsFL = function (lines) {
	...
	var res = this.parseRuleConditions(line); 
    return res
}
