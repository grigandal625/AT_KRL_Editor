...
AT_KRL_Parser.prototype.parseCoordinates = function (l) {
	...
	while (i < l.length) {
		...
		if (state == 'makePair' || state == "ini") {
			...
			x = '';
			y = '';
			while (isNaN(parseInt(l[i])) && i < l.length) {
				i++;
			}
			state = 'parseX';
			if (i == l.length) {
...
AT_KRL_Parser.prototype.parseStFzFact = function (line) {
	...
	function checkExSu(s) {
		...
		for (var i = exIndex; i < s.length; i++) {
			var count = 0;
			if (isNaN(parseInt(s[i])) && (s[i] != ',' || s[i] != '.') || (s[i] == ',' || s[i] == '.') && count > 0) {
...
AT_KRL_Parser.prototype.getToNextLogicOperation = function (subline, sNames, index, current) {
	...
	function isOperation(sl, n, i) {
		...
		var res = false;
		for (var k = 0; k < n.length; k++) {
			res = (res || ((s.indexOf(n[k], i) == i) && (n.indexOf(s.substr(i, n[k].length + 1)) == -1))) && (s.lastIndexOf(')') > s.lastIndexOf('УВЕРЕННОСТЬ'));
			if (res) {
				return n[k];
			}
		}
